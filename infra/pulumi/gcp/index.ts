...
const grafanaCloudLogsVersionConfigKeys = {
  url: "grafanaCloudLogsUrlVersion",
  username: "grafanaCloudLogsUsernameVersion",
  password: "grafanaCloudLogsPasswordVersion",
};
const grafanaCloudTracesVersionConfigKeys = {
  endpoint: "grafanaCloudTempoOtlpHttpEndpointVersion",
  username: "grafanaCloudTempoUsernameVersion",
  password: "grafanaCloudTempoPasswordVersion",
};
...
function createGrafanaCloudLogsSecret(
  ...
  const secretName = grafanaCloudLogsSecretName("gcp", environment, key);
  const secret = new gcp.secretmanager.Secret(
    secretName,
    {
      secretId: secretName,
  ...
  if (value) {
    new gcp.secretmanager.SecretVersion(
      `${secretName}-version`,
      {
        ...
        secretDataWo: value,
        secretDataWoVersion:
          trinityConfig.getNumber(grafanaCloudLogsVersionConfigKeys[key]) ?? 1,
      },
      { provider },
    );
  }
...
function createGrafanaCloudTracesSecret(
  ...
  const secretName = grafanaCloudTracesSecretName("gcp", environment, key);
  const secret = new gcp.secretmanager.Secret(
    secretName,
    {
      secretId: secretName,
  ...
  if (value) {
    new gcp.secretmanager.SecretVersion(
      `${secretName}-version`,
      {
        ...
        secretDataWo: value,
        secretDataWoVersion:
          trinityConfig.getNumber(grafanaCloudTracesVersionConfigKeys[key]) ?? 1,
      },
      { provider },
    );
  }
...
[...grafanaCloudLogsSecrets, ...grafanaCloudTracesSecrets].forEach(
  (secret, index) => {
    new gcp.secretmanager.SecretIamMember(
      `${clusterName}-external-secrets-read-grafana-cloud-extra-${index}`,
      {
        project,
        secretId: secret.secretId,
        role: "roles/secretmanager.secretAccessor",
...
export const grafanaCloudGcpLogsSecretNames = pulumi.all(
  grafanaCloudLogsSecrets.map((secret) => secret.secretId),
);
export const grafanaCloudGcpTracesSecretNames = pulumi.all(
  grafanaCloudTracesSecrets.map((secret) => secret.secretId),
);
