...
const secretsDemoSecret = new aws.secretsmanager.Secret(
  secretsDemoSecretName("aws", environment, secretNameSuffix),
  {
    name: secretsDemoSecretName("aws", environment, secretNameSuffix),
...
function createGrafanaCloudRemoteWriteSecret(
  ...
  const secretName = grafanaCloudRemoteWriteSecretName(
    "aws",
    environment,
    key,
    secretNameSuffix,
...
function createGrafanaCloudLogsSecret(
  ...
  const secretName = grafanaCloudLogsSecretName(
    "aws",
    environment,
    key,
    secretNameSuffix,
  ...
  const secret = new aws.secretsmanager.Secret(
    ...
    {
      name: secretName,
      description: `Grafana Cloud Loki ${key}.`,
      tags: {
        ...labels,
        component: "observability",
      },
...
function createGrafanaCloudTracesSecret(
  ...
  const secretName = grafanaCloudTracesSecretName(
    "aws",
    environment,
    key,
    secretNameSuffix,
  ...
  const secret = new aws.secretsmanager.Secret(
    ...
    {
      name: secretName,
      description: `Grafana Cloud Tempo ${key}.`,
      tags: {
        ...labels,
        component: "observability",
      },
  ...
  `${clusterName}-external-secrets-read-demo-secret`,
  {
    ...
    policy: pulumi
      .all([
        ...
        ...grafanaCloudLogsSecrets.map((secret) => secret.arn),
        ...grafanaCloudTracesSecrets.map((secret) => secret.arn),
      ])
      .apply((secretArns) =>
      JSON.stringify({
        Version: "2012-10-17",
...
export const grafanaCloudAwsLogsSecretNames = pulumi.all(
  grafanaCloudLogsSecrets.map((secret) => secret.name),
);
export const grafanaCloudAwsTracesSecretNames = pulumi.all(
