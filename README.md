...
and the AWS `ExternalSecret` manifests reference those same suffixed names. If
AWS reports another scheduled-deletion conflict, choose a new suffix and update
both the stack config and the AWS ExternalSecret `remoteRef.key` values:

```sh
pulumi -C infra/pulumi/aws config set trinity:secretNameSuffix <suffix>
```
...
`ClusterSecretStore` points at the same vault URL. If Azure reports that this
name also exists in deleted state, either purge or recover the deleted vault, or
choose another globally unique value and update both:

```sh
pulumi -C infra/pulumi/azure config set trinity:keyVaultName <unique-key-vault-name>
```

```yaml
# platform/secrets-demo/overlays/azure/cluster-secret-store.yaml
vaultUrl: https://<unique-key-vault-name>.vault.azure.net
```
...
pulumi -C infra/pulumi/gcp config set trinity:grafanaCloudLogsUrlVersion 2
pulumi -C infra/pulumi/gcp config set trinity:grafanaCloudLogsUsernameVersion 2
pulumi -C infra/pulumi/gcp config set trinity:grafanaCloudLogsPasswordVersion 2
pulumi -C infra/pulumi/gcp config set trinity:grafanaCloudTempoOtlpHttpEndpointVersion 2
pulumi -C infra/pulumi/gcp config set trinity:grafanaCloudTempoUsernameVersion 2
pulumi -C infra/pulumi/gcp config set trinity:grafanaCloudTempoPasswordVersion 2
```
